...
function manipulateUrl(url) {
  ...
  return {
    ...
    params: [...newUrl.searchParams.entries()], // Параметри URL у вигляді масиву пар [ключ, значення].
  };
}
...
function searchParamsURL(url) {
  ...
  const newUrl = new URL(url);
  let URLSearchParams = Object(newUrl.searchParams);
  let params = new Map();
  URLSearchParams.forEach((key, value) => params.set(key, value));
  return params;
}
...
function modifyURLParameters(url, params) {
  ...
  for (const item in params) {
    ...
    if (newUrl.searchParams.has(params[item])) {
...
function checkURLParameters(url, params) {
  ...
  // Повертаємо об'єкт з результатами.
   ...
   for(let param of params){
    result[param] = newUrl.searchParams.has(param);   
   }
   return result;
}
...
function processUrl(url, options) {
  ...
  //     urlObj.searchParams.append(item, "searchParams"[item]);
  //   }
  //  }
...
function manipulateQuery(url, options) {
  ...
  //   urlObj.searchParams.append([value]);
  //  }
  // if (options.has("delete")) {
  //   for (const item of options) {
  //     urlObj.searchParams.delete(delete[item]);
  //   }
...
function sortUrlParams(url) {
  ...
  // Повертаємо новий URL як рядок.
  let urlObj = new URL(url);
  let arr = Array.from(urlObj.searchParams.entries());
  let arrSort = arr.sort();
 
   urlObj.searchParams.delete("searchParams");
   //urlObj.searchParams.append([arrSort]);
   return urlObj.href;
 }
...
function getURLValues(url) {
  ...
  let arrKey = URLSearchParams.keys();
  let arrValue =  URLSearchParams.values();
...
function getUrlKeys(url) {
  ...
  // Повертаємо масив з ключами.
   ...
   let keys = objUrl.searchParams.keys();
   return keys;
}
